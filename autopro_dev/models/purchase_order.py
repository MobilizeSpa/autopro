...
class PurchaseOrder(models.Model):
    ...
    tracking_sale_order_id = fields.Many2one('sale.order', string='Pedido de venta')

    def send_message(self):
        self.ensure_one()
        ir_model_data = self.env['ir.model.data']
        try:
            template_id = ir_model_data.get_object_reference('autopro_dev', 'email_template_tracking_purchase_order')[1]
        except ValueError:
            template_id = False
        try:
